...
class EmployeeController extends Controller {
        ...
        if ($logged_user->can('view-details-employee'))
		{
            ...
            if (request()->ajax())
            {
                if ($request->department_id && $request->designation_id){
                    $employees = Employee::with('user:id,profile_photo,username','department:id,department_name', 'designation:id,designation_name')
                ...
                return datatables()->of($employees)
                    ...
                    {
                        if (isset($row->user->profile_photo))
                        {
                        ...
                        return "<div class='d-flex'>
                                        ...
                                        <div>"
                                            .$name.'</br>'.$username.'</br>'.$gender.'</br>'.$salary.'</br>'.$payslip_type;
                                        "</div>
                                    </div>";
                    })
                    ->addColumn('details', function ($row)
                    {
                        ...
                        return $department.'</br>'.$designation;
                    })
                    ...
                    ->addColumn('action', function ($data)
                    {
                        ...
                        if (auth()->user()->can('modify-details-employee'))
                        {
                            ...
                            $button .= '&nbsp;&nbsp;&nbsp;';
                            $button .= '<a class="download btn-sm" style="background:#FF7588; color:#fff" title="PDF" target="_blank" href="' . route('employees.pdf', $data->id ) . '"><i class="fa fa-file-pdf-o" aria-hidden="true"></i></a>';
                        }

                        return $button;
                    })
                    ->rawColumns(['name','details','contacts','action',])
                    ->make(true);
            }
            return view('employee.index', compact('departments','designations','roles'));
        }
        else
		{
	...
	public function store(Request $request)
	{
		...
		if ($logged_user->can('store-details-employee'))
		{
			if (request()->ajax())
			{
				$validator = Validator::make($request->only('first_name', 'email', 'contact_no', 'date_of_birth', 'gender',
					'username', 'role_users_id', 'password', 'password_confirmation', 'department_id', 'designation_id','attendance_type','joining_date'),
					[
						'first_name' => 'required',
	...
	public function show(Employee $employee)
	{
		if (auth()->user()->can('view-details-employee'))
		{
			...
			return view('employee.dashboard', compact('employee', 'countries', 'departments', 'designations', 'statuses', 'document_types', 'education_levels', 'language_skills', 'general_skills','roles'));
		}else
		{
			return response()->json(['success' => __('You are not authorized')]);
		}
