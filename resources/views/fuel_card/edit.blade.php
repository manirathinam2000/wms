...
<!-- Hero -->
        ...
        <div class="content content-full content-boxed">
          <!-- New Post -->
          <form action="{{ route('fuel_card.update') }}" method="POST" enctype="multipart/form-data">
          @csrf
                ...
                <div class="row justify-content-center push">
                  <div class="col-md-10">
                    <div class="mb-4">
                      ...
                      <input type="text" class="form-control" id="fuel_card_number" name="fuel_card_number" placeholder="Enter the Fuel Card Number.." value="{{ $data->fuel_card_number }}">
                    </div>
                    <div class="mb-4">
                      <label class="form-label" for="fuel_card_company">Fuel Card Company</label>
                      <input type="text" class="form-control" id="fuel_card_company" name="fuel_card_company" placeholder="Enter the Fuel Card Company.." value="{{ $data->fuel_card_company }}">
                    </div>
                    <div class="mb-4">
                      <label class="form-label" for="limits">Limit</label>
                      <input type="text" class="form-control" id="limits" name="limits" placeholder="Enter the limit.." value="{{ $data->limits }}">
                    </div>
                    <div class="mb-4">
                      <label class="form-label" for="expiry">Expiry</label>
                      <input type="text" class="form-control" id="expiry" name="expiry" placeholder="Enter the Expiry.." value="{{ $data->expiry }}">
                    </div>
                  </div>
                </div>
              </div>
